...
def hash_tensor(tensor: torch.Tensor):
    ...
    )  # Detach and convert to bytes
    h = hashlib.sha256(tensor_bytes)
    return h.hexdigest()
...
class SelfAttnProccessor:
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0).
    """

    def __init__(self, save_aux: bool = True, num_iterations = 300):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "AttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0."
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.save_aux:
            self.buffer.append({"key": key.detach().cpu(), "value": value.detach().cpu()})
        else:
            key = self.buffer[self.idx]["key"].to(key.device)
            value = self.buffer[self.idx]["value"].to(value.device)
            self.idx += 1
            self.idx %= self.num_iterations
